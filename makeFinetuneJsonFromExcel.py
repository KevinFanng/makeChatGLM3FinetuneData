...
def process_multiple_excels(input_folder, output_json_file):
    ...
    # 遍历指定文件夹下的所有Excel文件，文件格式见原始数据文件
        ...
        if filename.endswith(".xlsx"):
            ...
            for sheet_name in xls.sheet_names:
                ...
                for index, row in df.iterrows():
                    if index >= 4:    #具体根据实际excel数据具体位置进行调整
                        # 读取单元格数据
...
# 用法示例
excel_folder_path = 'E:\\MyWork\\原始数据所在目录'  # 替换成包含Excel文件的文件夹路径
output_json_file_path = 'train_piliang.json'  # 替换成你想要保存的 JSON 文件路径
